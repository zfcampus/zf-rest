...
namespace ZFTest\Rest\Plugin;

use ZF\Rest\HalCollection;
use ZF\Rest\HalResource;
use ZF\Rest\Link;
use ZF\Rest\MetadataMap;
...
class HalLinksTest extends TestCase
{
    ...
    public function testRendersEmbeddedResourcesInsideResourcesBasedOnMetadataMap()
    {
        ...
        $metadata = new MetadataMap(array(
            'ZFTest\Rest\Plugin\TestAsset\Resource' => array(
                'hydrator' => 'Zend\Stdlib\Hydrator\ObjectProperty',
                'route'    => 'hostname/resource',
            ),
            'ZFTest\Rest\Plugin\TestAsset\EmbeddedResource' => array(
                'hydrator' => 'Zend\Stdlib\Hydrator\ObjectProperty',
                'route'    => 'hostname/embedded',
            ),
            'ZFTest\Rest\Plugin\TestAsset\EmbeddedResourceWithCustomIdentifier' => array(
                'hydrator'        => 'Zend\Stdlib\Hydrator\ObjectProperty',
    ...
    public function testRendersEmbeddedCollectionsInsideResourcesBasedOnMetadataMap()
    {
        ...
        $metadata = new MetadataMap(array(
            'ZFTest\Rest\Plugin\TestAsset\Collection' => array(
                'is_collection'  => true,
    ...
    public function testWillNotAllowInjectingASelfRelationMultipleTimes()
    {
        ...
        $link = $links->get('self');
        $this->assertInstanceof('ZF\Rest\Link', $link);
    ...
    public function testResourcesFromCollectionCanUseHydratorSetInMetadataMap()
    {
        ...
        $metadata = new MetadataMap(array(
            'ZFTest\Rest\Plugin\TestAsset\ResourceWithProtectedProperties' => array(
    ...
    public function testInjectsLinksFromMetadataWhenCreatingResource()
    {
        ...
        $this->plugin->setMetadataMap($metadata);
        $resource = $this->plugin->createResourceFromMetadata($object, $metadata->get('ZFTest\Rest\Plugin\TestAsset\Resource'));
        $this->assertInstanceof('ZF\Rest\HalResource', $resource);
        $links = $resource->getLinks();
    ...
    public function testInjectsLinksFromMetadataWhenCreatingCollection()
    {
        ...
        $collection = $this->plugin->createCollectionFromMetadata(
            $set,
            $metadata->get('ZFTest\Rest\Plugin\TestAsset\Collection'
        ));
        $this->assertInstanceof('ZF\Rest\HalCollection', $collection);
